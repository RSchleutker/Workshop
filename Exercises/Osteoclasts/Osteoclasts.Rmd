...
2. `dplyr` bietet neben der Funktion `mutate()` noch Derivate dieser Funktion: `mutate_at(.vars, .funs)` erlaubt es, mehrere Spalten gleichzeitig zu bearbeiten. Dies kann man z.B. nutzen um gleichzeitig mehrere Variablen in Faktoren umzuwandeln. Die Funktion dazu lautet `factor`. Nutze `mutate_at()` und `factor`, um die Spalten `Genotype`, `Stimulant_1` und `Stimulant_2` in Faktoren umzuwandeln. Beachte hierbei, dass die Variablen-Namen innerhalb der Hilfsfunktion `vars()` übergeben werden müssen: `.vars = vars(Genotype, Stimulant_1, Stimulant_2)`.
...
Osteoclasts <- Osteoclasts %>%
  mutate_at(vars(Genotype, Stimulant_1, Stimulant_2), factor)
